...
export async function isCommitReachableAsync(repoPath, sha) {
    try {
        const result = await spawnAsync('git', ['-C', repoPath, 'cat-file', '-e', `${sha}^{commit}`], { encoding: 'utf-8' });
