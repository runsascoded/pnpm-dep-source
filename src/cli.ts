    ...
    verbose
      ? (() => {
          ...
          return Promise.all(projectEntries.map(([name, dep]) => {
            ...
            return fetchRemoteVersionsAsync(
              dep, name,
              dep.localPath ? resolve(projectRoot!, dep.localPath) : undefined,
              getInstalledVersion(projectRoot!, name) ?? undefined,
              cs,
            )
          }))
        })()
...
function extractDir(arg: string, nextArg?: string): { dir: string; consumesNext: boolean } | null {
  if (arg === '-C' || arg === '--dir') {
    return nextArg ? { dir: nextArg, consumesNext: true } : null
  }
  const eqMatch = arg.match(/^(?:-C|--dir)=(.+)$/)
  if (eqMatch) return { dir: eqMatch[1], consumesNext: false }
  // -Cwww (combined short option + value)
  if (arg.startsWith('-C') && arg.length > 2) return { dir: arg.slice(2), consumesNext: false }
  return null
}

function preParseDir(): void {
...
function argsWithoutFlags(): string[] {
  ...
  const result: string[] = []
  for (let i = 2; i < process.argv.length; i++) {
    if (skip.has(process.argv[i])) continue
    const dirResult = extractDir(process.argv[i], process.argv[i + 1])
    if (dirResult) {
      if (dirResult.consumesNext) i++
      continue
    }
